(function(){
  ...
  if(osForm){
    osForm.addEventListener("submit", function(e){ e.preventDefault(); });
  }
  ...
  // ---------- Formatação de moeda ----------
  const formatBRL = window.formatBRL || function(value){
  ...
  function validarDatas(){
    ...
    const invalido = Boolean(entrada && previsao && previsao < entrada);
    if(erroDatasEl) erroDatasEl.style.display = invalido ? "block" : "none";
    osPrevisaoInput.classList.toggle("invalid", invalido);
    return !invalido;
  }
  ...
  function validarTelefone(mostrarSeVazio){
    ...
    mostrarErro(telefoneInput, erroTelefoneEl, invalido && (mostrarSeVazio !== false));
    return !invalido;
  }
  ...
  function maskDocumento(valorBruto){
    ...
    let out = digitos.slice(0,2) + "." + digitos.slice(2,5) + "." + digitos.slice(5,8) + "/" + digitos.slice(8,12);
  ...
  function createLineRow(placeholderText){
    ...
    tr.innerHTML =
      '<td><input type="text" placeholder="' + placeholderText + '"></td>' +
  ...
  function setupLineTable(bodyId, addBtnId, placeholderText, onChangeCallback){
    ...
    function addRow(focusNew, prefill){
      ...
      if(prefill){
        ...
        priceInput.value = (prefill.preco || prefill.preco === 0) ? prefill.preco : "";
      }
      ...
      function handleEnter(e){
        ...
        e.preventDefault();
        const filled = descInput.value.trim() !== "" && priceInput.value.trim() !== "";
  ...
  const servicosTable = setupLineTable("servicosBody", "addServico", "Ex: Formatação e reinstalação do sistema", updateTotals);
  const pecasTable = setupLineTable("pecasBody", "addPeca", "Ex: Memória RAM 8GB DDR4", updateTotals);

  function sumTable(bodyId){
    const body = document.getElementById(bodyId);
    let total = 0;
    body.querySelectorAll(".price").forEach(input => { total += parsePrice(input); });
    return total;
  }
  ...
  function updateTotals(){
    ...
    document.getElementById("subtotalServicos").textContent = formatBRL(totalServicos);
    document.getElementById("subtotalPecas").textContent = formatBRL(totalPecas);
    document.getElementById("bdServicos").textContent = formatBRL(totalServicos);
  ...
  function addPhotoThumb(dataUrl){
    ...
    thumb.innerHTML = '<img src="' + dataUrl + '" alt="Foto do serviço">' +
  ...
  fotoInput.addEventListener("change", function(){
    Array.from(fotoInput.files).forEach(file => {
      ...
      reader.onload = function(e){ addPhotoThumb(e.target.result); };
  ...
  function getPhotos(){
    return Array.from(photoGrid.querySelectorAll(".photo-thumb")).map(t => t.dataset.src);
  }
  ...
  function getTableRows(bodyId){
    ...
    document.getElementById(bodyId).querySelectorAll("tr").forEach(tr => {
      ...
      if(desc !== "" || price > 0) rows.push({ descricao: desc, preco: price });
    });
    return rows;
  }
  ...
  function preencherFormulario(registro){
    ...
    document.getElementById("problemaDescricao").value = registro.problema || "";
    document.getElementById("diagnostico").value = registro.diagnostico || "";
    document.getElementById("orcamentoDescricao").value = registro.orcamento || "";

    document.getElementById("servicosBody").innerHTML = "";
    document.getElementById("pecasBody").innerHTML = "";
    (registro.servicos && registro.servicos.length ? registro.servicos : [{}]).forEach(r => servicosTable.addRow(false, r));
    (registro.pecas && registro.pecas.length ? registro.pecas : [{}]).forEach(r => pecasTable.addRow(false, r));
    ...
    if(badge){
      badge.textContent = "Editando O.S. nº " + (registro.osNumero || "sem número");
  ...
  if(editId && window.OSDatabase){
    ...
    if(registro){
      ...
      if(window.showToast) showToast("Ordem de serviço não encontrada. Criando uma nova.", "error");
  ...
  function salvarOS(mostrarToast){
    if(!window.OSDatabase){
      alert("Não foi possível acessar o banco de dados local (db.js). Verifique se o arquivo db.js está na mesma pasta do index.html.");
      return null;
    }
    if(window.OSDatabase.storageDisponivel && !window.OSDatabase.storageDisponivel()){
      alert("Seu navegador está bloqueando o armazenamento local (localStorage) para esta página.\n\n" +
    ...
    if(!validarDatas()){
      if(window.showToast) showToast("A previsão de entrega não pode ser antes da data de entrada.", "error");
      osPrevisaoInput.focus();
      return null;
    }
    const dados = collectData();
    const agora = new Date().toISOString();
    const registroExistente = registroAtualId ? OSDatabase.getById(registroAtualId) : null;
    const registro = Object.assign({}, dados, {
      id: registroAtualId || OSDatabase.generateId(),
      criadoEm: registroExistente ? (registroExistente.criadoEm || agora) : agora,
      atualizadoEm: agora
    });

    const salvo = OSDatabase.upsert(registro);
    if(!salvo){
      alert("Não foi possível salvar a ordem de serviço. O armazenamento local do navegador pode estar cheio ou bloqueado.");
      return null;
    }
    ...
    const badge = document.getElementById("editBadge");
    if(badge){
      badge.textContent = "Editando O.S. nº " + (salvo.osNumero || "sem número");
      badge.style.display = "flex";
    }

    if(mostrarToast !== false && window.showToast) showToast("Ordem de serviço salva com sucesso.", "success");
    return salvo;
  }
  ...
  document.getElementById("opcaoExcel").addEventListener("click", function(){
  ...
  document.getElementById("opcaoImprimir").addEventListener("click", function(){
    fecharModal();
    window.print();
  });
  ...
  document.getElementById("btnLimpar").addEventListener("click", function(){
    if(!confirm("Tem certeza que deseja limpar todos os campos desta ordem de serviço?")) return;
